...
On `action=deploy`, a `resolve-image` job SHALL set extra-vars for Ansible. It SHALL require a non-empty compose tag from `image_ref` or Environment `PORTAL_IMAGE`. A non-empty `image_http_url` input or Environment `IMAGE_HTTP_URL` SHALL be passed through for `docker load` and SHALL NOT replace the compose tag. It SHALL NOT run `docker build` or `npm run build`.

#### Scenario: Deploy with no image
- GIVEN `action=deploy`
- AND `image_ref` and `PORTAL_IMAGE` are both empty
