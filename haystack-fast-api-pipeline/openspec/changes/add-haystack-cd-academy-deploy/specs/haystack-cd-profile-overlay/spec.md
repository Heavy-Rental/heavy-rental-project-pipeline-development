...
Haystack Environment `academy` holds product Profile knobs. Infra `heavy-rental/haystack` holds AWS estate keys. CD overlays the former onto **guest** `.env` after SM map (ADR 0009). Setting those GitHub variables SHALL NOT change the GHCR image or `/app/.env` inside the pulled tag.
