...
#### Scenario: plaintext password in application properties
- GIVEN `src/main/resources/application.properties` contains `spring.datasource.password=` with a literal value (not `${ENV}` and not Jasypt `ENC(...)` / `{cipher}`)
