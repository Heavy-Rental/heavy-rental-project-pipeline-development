...
The Integration CI and Release callers SHALL pass database secrets with an explicit `secrets:` map. They SHALL NOT set `environment:` on the `uses:` job and SHALL NOT use `secrets: inherit`.
