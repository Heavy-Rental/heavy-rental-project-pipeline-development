...
- [x] 1.5 Align specs with Integration Check, Fast Feedback reuse, both callers’ explicit `REST_API_DB_*` maps, and split Semgrep
...
- [x] 2.3 Integration and Release callers both pass `REST_API_DB_*` via an explicit map

## 3. Verify

- [x] 3.1 Job `name:` values in the walkthrough match YAML (`Integration Check` on CI, `Integration` on Fast Feedback and Release)
- [x] 3.2 Secret names match both callers’ `secrets:` maps (Repository secrets)
