...
3. **Two secret families.** Integration QC: Environment `integration` + `REST_API_DB_*` for a local Docker Postgres. Release QC: Environment `production` + `REST_API_CLOUD_DB_*` for a **local** Docker Postgres plus a cloud JDBC URL artifact (no password). Neither is guest CD config.
...
| Risk | Mitigation |
| --- | --- |
| Confusing `REST_API_CLOUD_DB_*` with guest SM | PREPARE + `rest-ci-scope` + ADR 0005 |
