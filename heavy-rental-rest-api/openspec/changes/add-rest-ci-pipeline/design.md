...
3. **Same `REST_API_DB_*` names, two Environments.** Integration QC: Environment `integration` plus an explicit caller secrets map from **Repository** secrets. Release QC: Environment `production` only (no caller map). Both start a **local** Docker Postgres. `REST_API_DB_URL` is derived. Neither is guest CD config. No `secrets: inherit`.
...
| `environment:` on a `uses:` job | Invalid; Integration forwards Repository secrets via an explicit map; Release QC reads Environment `production` on the reusable job; no inherit |
| Empty Integration QC secrets | Repository secrets required for the caller map (a `uses:` job cannot read Environment secrets) |
