...
Copy estate `guest_base` + `rest`. Pipeline `resolve-image` chooses tag or tar. Ansible over SSM, `--limit rest`. Verify is SSM `GET :8080/actuator/health` **2xx** (ALB `tg-rest` matcher `200-299`; not `GET /` 401).
