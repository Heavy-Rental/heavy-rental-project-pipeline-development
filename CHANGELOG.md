...
- Portal nginx `/api` omits `Origin` (`proxy_set_header Origin ""`). Spring `APP_CORS_ALLOWED_ORIGINS` is for **direct** REST ALB browser calls, not the same-origin hairpin. Living specs (root + portal + REST indexes, `portal-cd` / `portal-ci` / `rest-cd`), OpenSpec/SPDD for portal CD Ansible, operator BOOTSTRAP/PREPARE, REST BOOTSTRAP, and feasibility as-built snippets now match: Release nginx is `try_files` only; guest `nginx-default.conf.j2` adds `/api`. `GET /api/auth/getBearerToken` 403 `Invalid CORS request` plus login 401 means Origin was forwarded (or the address bar was not `http://<portal_alb_dns>/`). Infra ADR 0018 and portal ADRs 0003 / 0008 amended.
